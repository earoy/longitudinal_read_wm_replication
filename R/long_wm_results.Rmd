...
full_df_harm = read_csv('/home/users/ethanroy/vanderbilt_colab/data/full_df_harm_rev.csv') 

# Load harmonized tract profile data
full_profile_harm_df = read_csv('/home/users/ethanroy/vanderbilt_colab/data/full_profile_harm_df_rev.csv')

# Load image qc metrics
qc_metric_df = read_csv('/home/users/ethanroy/vanderbilt_colab/data/qc_metric.csv')

# combine diffusion and qc data frames
full_df_harm = full_df_harm %>% inner_join(qc_metric_df, by=c('subjectID','sessionID'))
full_profile_harm_df = full_profile_harm_df %>% inner_join(qc_metric_df, by=c('subjectID','sessionID'))
...
behavioral = read_excel('/home/users/ethanroy/vanderbilt_colab/data/LD4_Behavioral.xlsx')%>% 
  mutate( has_behav = if_else(is.na(`WJ-III Basic Reading W Score`), 0, 1),
          WJ_read = as.numeric(`WJ-III Basic Reading W Score`),
          WJ_read_std = as.numeric(`WJ-III Basic Reading Standard Score`),
          WJ_math = as.numeric(`WJ-III Applied Problems W Score`)) %>% 
  select(Participant, Visit, has_behav,Age, WJ_read, WJ_read_std, WJ_math) %>% 
  rbind(read_excel('/home/users/ethanroy/vanderbilt_colab/data/RC3_Behavioral.xlsx') %>% 
          mutate( has_behav = if_else(is.na(`WJ-IV Basic Reading Skills W Score`), 0, 1),
                  ...
                  WJ_read_std = as.numeric(`WJ-IV Basic Reading Skills Standard Score`),
...
# get count of concurrent behavioral/neuroimaging sessions
behavioral %>% 
...
# Missing Math data
arc_l_df_harm %>% 
  select(subjectID,sessionID,WJ_math_std) %>% 
  unique() %>% 
  mutate(missing_math = if_else(is.na(WJ_math_std), T, F)) %>% 
  group_by(missing_math,sessionID) %>% 
  summarise(count = n())

# Variability in Age
arc_l_df_harm %>% 
  select(subjectID,sessionID,Age) %>% 
  unique() %>% 
  group_by(sessionID) %>% 
  summarise(mean_age = mean(Age, na.rm=T),
            std = sd(Age, na.rm=T))
...
# Within-Individual changes in reading skill track changes in the white matter properties of the left arcuate
...
anova(m0,m0.1, test='Chisq') # m0.1 fits better
...
anova(m0.1,m0.2, test='Chisq') # m0.2 fits better
...
anova(m0.3,m0.2, test = 'Chisq') # m0.3 fits better
...
anova(m0.3, m0.4, test='Chisq')  # m0.4 fits better
...
anova(m0.4, m0.5, test='Chisq') # m0.5 fits better
gam.check(m0.5) # looks okay
...
gam.check(m1.1) # looks okay
anova(m0.5,m1.1, test='Chisq') # fits better

summary(m1.1)


start_value_rho(m1.0, plot=T)
acf_resid(m1.0)

start_value_rho(m1.1, plot=T)
acf_resid(m1.1)


```

### Add node-wise interactions to model

```{r}

m1.1.0 = bam(dti_md ~  initial_age +
                    Sex +
                  raw_neighbor_corr+
                  raw_num_bad_slices+
                  max_rel_translation+ 
...
# anova(m1.1, m1.1.1, test='Chisq') #  better
gam.check(m1.1.1, rep=100) 

summary(m1.1.1)


preds = predict.gam(m1.1.1, se.fit=T,newdata=arc_l_df_harm)

arc_l_df_harm$m1_pred = preds$fit
arc_l_df_harm$m1_pred_se = preds$se.fit
arc_l_df_harm$m1_resid = resid(m1.1.1, newdata=arc_l_df_harm)
dfs1 <- smooth_estimates(m1.1.1) %>% add_confint(coverage=.68)
...
rho2=start_value_rho(m1.1_resid)

m1.1_resid2 = bam(m1_resid ~initial_age +
                    Sex +
                  raw_neighbor_corr+
                  raw_num_bad_slices+
                  max_rel_translation+ 
...
reg_data = arc_l_df_harm %>% 
    filter(subjectID %in% sub_df_3obs$subjectID) %>%
  group_by(subjectID,sessionID,tractID,raw_neighbor_corr,raw_num_bad_slices,max_rel_translation) %>% 
...
# Discrete Time 
lme_mod = lmer(demean_read ~ sessionID + demean_md + initial_age + mean_md  +raw_neighbor_corr+raw_num_bad_slices+ (sessionID||subjectID),data=reg_data)


# Continuous Time
lme_mod_age_c = lmer(demean_read ~ age_c + demean_md + initial_age + mean_md  +raw_neighbor_corr+raw_num_bad_slices+ (age_c|subjectID),
                  data=reg_data)
...
# Compute slopes for reading score development
...
# Compute slopes for MD development in left arcuate
...
# compute correlation between slopes
...
# plot within-individual reading against within-individual MD
...
# plot reading slope against MD slope
...
# to find t-statistics match the statistics from this output with sumRes
# summary(Res1$output$temporal$mean_dtiMD)
...
full_profile_raw_df = read_csv('/home/users/ethanroy/vanderbilt_colab/data/tract_profiles_filtered_rev.csv')
...
# Plot Reading and Math scores at each time point
behavioral %>% 
  mutate(Visit = as.factor(Visit)) %>% 
  ggplot(aes(x=WJ_read, y=WJ_math)) +
...
rho2=start_value_rho(m2.0)


## Add AR(1) structure to model
m2.0 = bam(dti_md ~  initial_age +
                    Sex +
                  raw_neighbor_corr+
                  raw_num_bad_slices+
...
dfs1 <- smooth_estimates(m2.0) %>% add_confint(coverage=.68)

arc_r_df_harm$m1_pred = predict(m2.0, newdata=arc_r_df_harm)
preds = predict(m2.0, se.fit=T,newdata=arc_r_df_harm)
arc_r_df_harm$m1_pred = preds$fit
arc_r_df_harm$m1_pred_se = preds$se.fit
...
right_arc_profile = arc_r_df_harm %>% 
  mutate(`Within-Individual Reading` = as.factor(ntile(WJ_read_C, n=4)),
         m1_pred = m1_pred*1000,
         m1_pred_se = m1_pred_se*1000) %>% 
  group_by(nodeID, `Within-Individual Reading`) %>% 
  summarise(mean_pred = mean(m1_pred, na.rm = TRUE),
            se = sd(m1_pred, na.rm = TRUE) / sqrt(n()),
            se = mean(m1_pred_se, na.rm = TRUE)) %>% 
  drop_na() %>% 
  mutate(read_change_num = as.numeric(`Within-Individual Reading`),
         `Within-Individual Reading` = case_when(
           `Within-Individual Reading` == '1' ~ 'Lowest',
           `Within-Individual Reading` == '2' ~ 'Low',
           `Within-Individual Reading` == '3' ~ 'High',
           `Within-Individual Reading` == '4' ~ 'Highest')) %>% 
  mutate(`Within-Individual Reading` = factor(
    `Within-Individual Reading`, levels = c("Highest", "High", "Low", "Lowest"))) %>% 
  ggplot(aes(x = nodeID, y = mean_pred, group = `Within-Individual Reading`)) +
  geom_line(aes(color = `Within-Individual Reading`)) +
  geom_ribbon(aes(fill = `Within-Individual Reading`, 
                  ymin = mean_pred - se, ymax = mean_pred + se), alpha = 0.2) +
  theme_bw() +
  ylab(expression("Predicted Mean Diffusivity ("*mu*"m"^2*"/msec)")) +
  xlab("Position Along Tract") +
  scale_color_d3() +
...
right_arc_mean_read = arc_r_df_harm %>% 
  mutate(`Within-Individual Reading` = as.factor(ntile(WJ_read_C, n = 4)),
         m1_pred = m1_pred * 1000,
         m1_pred_se = m1_pred_se * 1000) %>% 
  group_by(`Within-Individual Reading`) %>% 
  summarise(mean_pred = mean(m1_pred, na.rm = TRUE),
            se = sd(m1_pred, na.rm = TRUE) / sqrt(n()),
            se = mean(m1_pred_se, na.rm = TRUE)) %>% 
  drop_na() %>% 
  ...
  mutate(`Within-Individual Reading` = factor(
    `Within-Individual Reading`, levels = c("Highest", "High", "Low", "Lowest"))) %>% 
  ggplot(aes(x = `Within-Individual Reading`, y = mean_pred, color = `Within-Individual Reading`)) +
  geom_point() +
  geom_errorbar(aes(ymin = mean_pred - se, ymax = mean_pred + se),width = 0.25) +
  theme_bw() +
  ylab(expression("Predicted Mean Diffusivity ("*mu*"m"^2*"/msec)")) +
  xlab("") +
  scale_color_d3() +
...
(right_arc_profile + ( right_arc_mean_read/age_smooth)) + 
  plot_layout(guides = "collect")+ 
  plot_annotation(tag_levels = 'A') & 
  theme(legend.position = "bottom",
...
# ACF plots to examine auto-correlation
acf_resid(m1.1.0)
acf_resid(m1.1.1)

# Plot individual residuals across each time point
subjects = unique(arc_l_df_harm$subjectID)

mean_resid_df = arc_l_df_harm %>% 
  group_by(nodeID,sessionID) %>% 
  summarise(mean_resid = mean(m1_resid, na.rm=T))%>% 
  mutate(sessionID = case_when(sessionID=="1"~ 'Timepoint 1',
                               sessionID=="2"~ 'Timepoint 2',
                               sessionID=="3"~ 'Timepoint 3',
                               sessionID=="4"~ 'Timepoint 4'))
...
arc_l_df_harm %>% 
  mutate(sessionID = case_when(sessionID=="1"~ 'Timepoint 1',
                               ...
                               sessionID=="4"~ 'Timepoint 4')) %>% 
  ggplot()+
  geom_line(aes(x=nodeID, y=m1_resid,group=subjectID))+
  geom_line(data=mean_resid_df, aes(x=nodeID, y = mean_resid),color='red')+
  facet_grid(.~sessionID)+
  ylab("Residual")+
  theme_bw()
...
### Use age as operationalization of time
